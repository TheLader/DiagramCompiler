...
class Block:
    # Статичний список для збору всіх створених блоків
    all_blocks = []
    ...
    def __init__(self, canvas, x, y, type):
        ...
        # Додаємо цей екземпляр до списку всіх блоків
    ...
    def createOutput(self):
        # Паралелограм для output (інверсія input)
        offset = 20
        x1, y1 = self.x, self.y
    ...
    def createArrow(self, block, text=""):
        import Arrow  # уникаємо циклічного імпорту
        arr = Arrow.Arrow(self.canvas, self, block, text)
        self.arrow.append(arr)
    ...
    def setCondition(self):
        text = simpledialog.askstring("Вписати текст", "Умова (наприклад: x > 0)")
        self.condition = text
        self.updateText()
    ...
    def updateText(self):
        ...
        x, y = self.get_center()
        if self.type == "variable":
            display = f"{self.variableName}={self.variableValue}" if self.variableName and self.variableValue else ""
        elif self.type == "input":
            display = f"input({self.inputVariable})" if self.inputVariable else "Input"
        elif self.type == "operation":
            display = self.operationText if self.operationText else "Operation"
        elif self.type == "output":
            display = f"print({self.outputVariable})" if self.outputVariable else "Output"
        elif self.type == "condition":
            display = self.condition if self.condition else "Condition"
        else:
            display = self.type
        self.text = self.canvas.create_text(x, y, text=display)

    def delete(self):
        # Видаляємо стрілки, що ведуть до цього блоку
        for connectedFromBlock in self.connectedFromBlocks:
        ...
        # Якщо цей блок має connectedToBlock, видаляємо звʼязок
        if self.connectedToBlock:
            self.connectedToBlock.connectedFromBlocks.remove(self)

        # Видаляємо сам елемент і текст
        self.canvas.delete(self.item)
        self.canvas.delete(self.text)

        # Видаляємо стрілки, що виходять з цього блоку
        for arrow in list(self.arrow):
            arrow.delete()
            self.arrow.remove(arrow)
    ...
    def get_border_point(self, other_center):
        ...
        # Якщо ромб або паралелограм
            ...
            def line_intersection(p1, p2, p3, p4):
                ...
                if (between(x1, x2, px) and between(y1, y2, py)
                        and between(x3, x4, px) and between(y3, y4, py)):
                    return (px, py)
                return None
